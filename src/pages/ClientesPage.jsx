...
const NIVELES = {
  ...
  plata: { color: '#9ca3af', bg: 'bg-gray-50', emoji: '🥈', label: 'Plata', descuento: 5 },
  oro: { color: '#d97706', bg: 'bg-yellow-50', emoji: '🥇', label: 'Oro', descuento: 8 },
  platino: { color: '#7c3aed', bg: 'bg-purple-50', emoji: '💎', label: 'Platino', descuento: 12 },
}
    ...
    <div className={`p-6 ${bg} min-h-full`}>
      ...
      <div className={`${card} overflow-hidden`}>
        <div className={`p-4 border-b ${modoOscuro ? 'border-gray-700' : 'border-gray-100'}`}>
          <input value={busqueda} onChange={e => setBusqueda(e.target.value)} placeholder="Buscar por nombre, teléfono o NIT..."
          ...
          <table className="w-full">
            ...
            <tbody className={`divide-y ${modoOscuro ? 'divide-gray-700' : 'divide-gray-100'}`}>
              {clientesFiltrados.map(c => {
                ...
                return (
                  <tr key={c.id} className={`transition-colors cursor-pointer ${modoOscuro ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}`} onClick={() => setModalDetalle(c)}>
          ...
          <div className={`rounded-2xl p-6 w-full max-w-md shadow-2xl animate-slide-up ${modoOscuro ? 'bg-gray-800' : 'bg-white'}`}>
            ...
            <div className="space-y-3">
              {[
                ...
                <input key={f.key} value={form[f.key]} onChange={e => setForm({...form, [f.key]: e.target.value})} placeholder={f.placeholder} type={f.type || 'text'} className={inputCls} />
              ))}
            </div>
            <div className="flex gap-3 mt-4">
